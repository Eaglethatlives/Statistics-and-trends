...
"""
"""
This code gives a graphical representation of makeup brand color properties using statistical and graphical methods.
It includes functions for data preprocessing, visualization, and statistical analysis.
...
def plot_relational_plot(df):
    """Creates scatter plots to show the relationships between Hue (H), Saturation (S)."""
    fig, ax = plt.subplots(2, 2, figsize=(12, 10))  
    ax = ax.flatten()  
    plot_pairs = [('H', 'S'), ('H', 'V'), ('S', 'V')]  
    titles = ['MAC Brands Hue (H) vs. Saturation (S)', 'Fenty Brands Hue (H) vs. Value (V)', 'Revlon Brands Saturation (S) vs. Value (V)']
...
def plot_categorical_plot(df):
    """This section creates categorical plots: bar plots and a pie chart for makeup products for dataset."""
    ...
    plt.figure(dpi=144)
    plt.pie(brands, labels=labels, autopct='%1.1f%%', startangle=140, colors=['#ff0000', '#66b4ff', '#99ff99', '#ffcc99', '#445bbb'])
...
def plot_statistical_plot(df):
    """ This section creates histograms and shows the distribution of Hue, Saturation, and Lightness across different brands."""
...
def statistical_analysis(df, col: str):
    """Calculates statistical moments (mean, standard deviation, skewness, and kurtosis) for a given column."""
    mean = df[col].mean()
    stddev = df[col].std()
    skew = df[col].skew()
...
def writing(moments, col):
    ...
    print(f'Mean = {mean:.2f}, Standard Deviation = {stddev:.2f}, Skewness = {skew:.2f}, Excess Kurtosis = {excess_kurtosis:.2f}.')
    
    skewness_desc = "right-skewed" if skew > 2 else "left-skewed" if skew < -2 else "not skewed"
    kurtosis_desc = "leptokurtic" if excess_kurtosis > 1 else "platykurtic" if excess_kurtosis < -1 else "mesokurtic"
    
    print(f'The data is {skewness_desc} and {kurtosis_desc}.')

def main():
    """Main function that executes data processing, visualization, and statistical analysis."""
