...
def plot_relational_plot(df):
    ...
    for i, ((x_col, y_col), title, color) in enumerate(zip
        (plot_pairs, titles, colors)):
        sns.scatterplot(x=df[x_col],
        y=df[y_col], alpha=0.5, color=color, ax=ax[i])
    ...
    numeric_cols = ['H','S','V','L']
    df_numeric = df[numeric_cols].dropna()
    fig = corner(df_numeric, 
    labels=numeric_cols, show_titles=True,
    title_fmt=".2f", color='blue')
    return
...
def plot_categorical_plot(df):
    ...
    data=df,hue='group',palette='viridis', ax=ax)
    ax.set_title("Number of MakeUp Products by Brand",
    fontsize=16)
    ...
    sns.barplot(x='brand', y='L',hue='group', data=df, 
    errorbar=None, palette=["pink", "yellow",
    "blue", "purple", "black", "brown", "red", "orange"])
    plt.title("Bar Plot for Average Lightness forMakeUp Brands",
    fontsize=16)
    ...
    plt.figure(dpi=144)
    plt.pie(brands, labels=labels, autopct='%1.1f%%', 
    startangle=140, colors=
...
def writing(moments, col):
    """interprets statistical moments in a human-readable format."""
