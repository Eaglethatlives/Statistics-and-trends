...
def plot_relational_plot(df):
    """
    Create scatter plots for relational analysis.
    ...
    titles = [
        'MAC Brands Hue (H) vs. Saturation (S)',
        'Fenty Brands Hue (H) vs. Value (V)',
        'Revlon Brands Saturation (S) vs. Hue (H)'
    ]
...
def plot_categorical_plot(df):
    """
    Create count plots, bar plots, and pie charts for categorical analysis.

    Parameters:
        df (pd.DataFrame): The DataFrame containing the data.
    """
    # Count plot of products by brand
    ...
    # Bar plot of average lightness by brand
    fig, ax = plt.subplots(figsize=(12, 8))
    sns.barplot(
        x='brand', y='L', hue='group', data=df, errorbar=None,
        palette=["pink", "yellow", "blue", "purple", "black", "brown", "red", "orange"]
    )
...
def statistical_analysis(df, col: str):
    """
    Calculate statistical moments (mean, standard deviation, skewness, and kurtosis).
    ...
    Returns:
        tuple: A tuple containing (mean, stddev, skewness, excess_kurtosis).
    """
    mean = df[col].mean()
    stddev = df[col].std()
...
def preprocessing(df):
    """
    Preprocess the DataFrame (e.g., handle missing values, clean data).
...
def writing(moments, col):
    """
    Write and interpret statistical moments in a human-readable format.

    Parameters:
        moments (tuple): A tuple containing (mean, stddev, skewness, excess_kurtosis).
